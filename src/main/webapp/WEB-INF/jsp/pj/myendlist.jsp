...
<header>
  ...
  <nav>
    ...
    <span> ｜ </span>
    <span><a id="now" href="/wg/myendlist">관람한 공연</a></span>
    <span> ｜ </span>
...
<c:when test="${pageInfo.navigateLastPage>5}">
	<div class="num"> <a href="/wg/myendlist?pageNum=${pageInfo.navigateFirstPage}">${pageInfo.navigateFirstPage}</a></div>
	<div class="num"> ... </div>
	<c:choose>
		<c:when test="${pageInfo.prePage<1}"></c:when>
		<c:otherwise>
		<div class="num"><a href="/wg/myendlist?pageNum=${pageInfo.prePage}">${pageInfo.prePage}</a></div>
		</c:otherwise>
	</c:choose>
	<div class="num"><a class="now" href="/wg/myendlist?pageNum=${pageInfo.pageNum}">${pageInfo.pageNum}</a></div>
		<c:choose>
		<c:when test="${pageInfo.nextPage>pageInfo.navigateLastPage || pageInfo.nextPage==0 }"></c:when>
		<c:otherwise>
		<div class="num"><a href="/wg/myendlist?pageNum=${pageInfo.nextPage}">${pageInfo.nextPage}</a></div>
		</c:otherwise>
	</c:choose>
	<div class="num"> ... </div>
	<div class="num"> <a href="/wg/myendlist?pageNum=${pageInfo.pages}">${pageInfo.pages}</a></div>
	</c:when>
	<c:otherwise>
		<c:forEach var="pageNum" items="${pageInfo.navigatepageNums}">
			<c:choose>
				<c:when test="${pageInfo.pageNum==pageNum}">
					<div class="num"><a class="now" href="/wg/myendlist?pageNum=${pageNum}">${pageNum}</a></div>
				</c:when>
				<c:otherwise>
					<div class="num"><a href="/wg/myendlist?pageNum=${pageNum}">${pageNum}</a></div>
				</c:otherwise>
			</c:choose>
		</c:forEach>
	</c:otherwise>
