...
<script type="text/javascript">
	...
	function addendperformdeletewill(){
		...
		$.ajax({ 
		   url:'/wg/mylist/will/update2', 
		   method:'post',
		   cache: false,  
		   data: serData,
		   dataType: 'json',
		   success: function(res) {
			   alert(res.deletewilladdendperform ? '수정 성공' : '수정 실패');
			   if(res.deletewilladdendperform) location.href='/wg/myendlist';
		   },
	...
	function updatewillperform(){
		var serData = $('#addwillPerform').serialize();
		
	$.ajax({ 
	   url:'/wg/mylist/will/update', 
	   method:'post',
	   cache: false,  
	   data: serData,
	   dataType: 'json',
	   success: function(res) {
		   alert(res.updatewillperform ? '수정 성공' : '수정 실패');
		   if(res.updatewillperform) location.href='/wg/mywilllist';
	   },
	   error: function(xhr, status, err){
	...
	function addhopeperformdeletewill(){
		if(!confirm('기존의 정보는 초기화됩니다! 계속 하시겠어요?')) {
			return false;
		}
		...
		$.ajax({ 
		   url:'/wg/mylist/will/update3', 
		   method:'post',
		   cache: false,  
		   data: serData,
		   dataType: 'json',
		   success: function(res) {
			   alert(res.deletewilladdhopeperform ? '수정 성공' : '수정 실패');
			   if(res.deletewilladdhopeperform) location.href='/wg/myhopelist';
		   },
		   error: function(xhr, status, err){
			   if(serData.wp_uid==null) {
					alert('로그인이 필요한 서비스입니다')
...
<body>
<header>
  <h2>관람 예정 공연</h2><a href="/wg/home"><img src="/images/home.png"></a>
  <nav>
...
<c:forEach var="p" items="${willperform.perform}">
	...
	<p class="main">관람일 <a class="info"> ${willperform.wp_date}</a></p>
	<p class="main">관람시간 <a class="info"> ${willperform.wp_time}</a></p>
	<p class="main">좌석 <a class="info">${willperform.wp_seat}</a></p>
	</div>
...
<div class="select">
	...
	<input type="radio" id="will" name="opt" value="will" checked="checked"
...
<div id="opt_end"  style="display:none;" >
	<form id="updateendPerform" method="post" onsubmit="return addendperformdeletewill();">
		<input type="hidden" name="wp_num" value="${willperform.wp_num}">
		<input type="hidden" name="ep_pernum" value="${willperform.wp_pernum}">
		<div>
		<label for="ep_date">관람일</label>
			<input type="date" id="ep_date" name="ep_date"  value="${willperform.wp_date}">
		</div>
		<div>
		<label for="ep_seat">좌석</label>
			<input type="text" id="ep_seat" name="ep_seat" value="${willperform.wp_seat}" >
		</div>
		<div>
		<label for="ep_time">관람시간</label>
			<input type="time" id="ep_time" name="ep_time" value="${willperform.wp_time}">
		</div>
		<div>
...
<div class="willform">
<div id="opt_will" >
	<form id="addwillPerform" method="post" onsubmit="return updatewillperform();">
		<input type="hidden" name="wp_num" value="${willperform.wp_num}">
		<div>
		<label for="wp_date">관람일</label>
			<input type="date" id="wp_date" name="wp_date" value="${willperform.wp_date}">
		</div>
		<div>
		<label for="wp_date">관람시간</label>
			<input type="time" id="wp_time" name="wp_time" value="${willperform.wp_time}">
		</div>
		<div>
		<label for="wp_seat">좌석</label>
			<input type="text" id="wp_seat" name="wp_seat" value="${willperform.wp_seat}" >
		</div>
...
<div id="opt_hope"  style="display:none;">
	<form id="addhopePerform" method="post" onsubmit="return addhopeperformdeletewill();">
		<input type="hidden" name="wp_num" value="${willperform.wp_num}">
		<input type="hidden" name="hp_pernum" value="${willperform.wp_pernum}">
		<div>
